...
$(document).ready(function() {
    ...
    // handle special keys on key down
...
function start_stats() {
    data.start_time = data.start_time || Math.floor(new Date().getTime() / 1000);
}
...
function update_stats() {
    if (data.start_time) {
        ...
        data.ratio = Math.floor(
            data.hits_correct / (data.hits_correct + data.hits_wrong) * 100
        );
        data.hpm = Math.floor(
            (data.hits_correct + data.hits_wrong) / (current_time - data.start_time) * 60
        );
        if (!isFinite(data.hpm)) { data.hpm = 0; }
    }
}
...
function keyHandler(e) {
    ...
    if (data.word_index >= data.word.length/2) {
        setTimeout(move_word, 100);
    }
...
// move the word with one char
function move_word(){
    data.chars_index ++;
    data.word = data.chars.substring(data.chars_index,data.chars_index+data.word_length);
    data.word_index --;
    data.keys_hit = data.keys_hit.substring(1);
    data.word_errors.shift();
    update_stats();
...
function render_stats() {
    $("#stats").text([
        "hits per minute: ", data.hpm, " ",
        "correctness: ", data.ratio, "%"
    ].join(""));
}
...
function control() {
    $("#control > #restart").click(function(e){
        start();
    });
...
function setFontSize(fontSize){
    $("#word").css("font-size",data.font_size+"px");
    $(".keys-hit").css("font-size",data.font_size+"px");
}

// minus X till Y
function minusXTillY(num,x,y) {
    return num>y?num-x:y;
}

// plus X till Y
function plusXTillY(num,x,y) {
    return num<y?num+x:y;
}
